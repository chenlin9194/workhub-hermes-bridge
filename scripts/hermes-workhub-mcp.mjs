...
const milestoneCreateSchema = {
  ...
  description: z.string().optional(),
  stage: z.enum(["planning", "concept", "plan", "verification"]).describe("Required WorkHub milestone stage."),
  planType: z.enum(["milestone", "requirement", "development", "test", "release", "management", "other"]).optional().describe("Defaults to milestone."),
  dateMode: z.enum(["point", "range"]).optional().describe("Usually derived from planType."),
  status: z.enum(["planned", "in_progress", "done", "delayed", "cancelled"]).optional().describe("Defaults to planned."),
